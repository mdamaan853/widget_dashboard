...
import useSideBarNavItems from "./SiderBarNavItems"

const SidebarContent = props => {
  const ref = useRef();
  const SiderBarNavItems = useSideBarNavItems();
      ...
      <SimpleBar style={{ maxHeight: "100%" }} ref={ref}>
        <div id="sidebar-menu">
          <ul className="metismenu list-unstyled" id="side-menu">
            ...
            {SiderBarNavItems.map(item => (
              <li key={item.sideBarDisplayName}>
                <NavLink className="waves-effect" to={item.path}>
                  <item.SideBarIconComponet />

                  {/* <span className="badge rounded-pill bg-primary float-end">
                  2
                </span> */}
                  <span>{props.t(item.sideBarDisplayName)}</span>
                </NavLink>
              </li>
            ))}
            {/* <li>
